...
FOR EACH ROW
BEGIN
  IF To_Number(To_Char(:old.datum_zaposlenja, 'YYYY')) <= 1998 THEN
...
BEGIN
  ...
  IF dan IN ('MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY') AND sat = 16 AND minuta >= 23 OR sat > 16 AND sat < 23 OR sat = 23 AND minuta <= 16 THEN
    Raise_Application_Error(-20001, 'Zabranjen unos u toku ovog vremena!');
  ELSIF dan IN ('SATURDAY', 'SUNDAY') AND sat = 6 AND minuta >= 34 OR sat > 6 AND sat < 23 OR sat = 23 AND minuta <= 56 THEN
    Raise_Application_Error(-20002, 'Zabranjen unos u toku ovog vremena!');
  END IF;
END;
/
